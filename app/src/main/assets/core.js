...
function friendlyApiError(data,status){
  ...
  if(Array.isArray(detail))detail=detail.map(x=>x?.msg||x?.message||'Donnée invalide').join(' • ');
  else if(detail&&typeof detail==='object')detail=detail.message||detail.msg||JSON.stringify(detail);
  let message=detail||data?.msg||data?.message||data?.error_description||(typeof data?.error==='string'?data.error:data?.error?.message)||'';
  const low=String(message).toLowerCase();
  if(code.includes('weak_password')||low.includes('password')&&(low.includes('weak')||low.includes('least one')||low.includes('characters')))return 'Mot de passe trop faible : utilisez au moins 12 caractères avec majuscule, minuscule, chiffre et symbole.';
  if(code.includes('invalid_credentials')||low.includes('invalid login credentials'))return 'E-mail ou mot de passe incorrect.';
  if(code.includes('email_not_confirmed')||low.includes('email not confirmed'))return 'Confirmez votre adresse e-mail avant de vous connecter.';
  if(code.includes('user_already_exists')||low.includes('already registered')||low.includes('already exists'))return 'Un compte existe déjà avec cette adresse e-mail.';
  if(code.includes('email_address_invalid')||low.includes('invalid email'))return 'Adresse e-mail invalide.';
  if(message)return String(message);
  if(Number(status)===422)return 'Données invalides. Vérifiez le formulaire puis réessayez.';
  return `Erreur ${status}`;
}
async function parse(res){const text=await res.text();let data={};try{data=text?JSON.parse(text):{}}catch{data={detail:text}}if(!res.ok)throw Object.assign(new Error(friendlyApiError(data,res.status)),{status:res.status,data});return data}
