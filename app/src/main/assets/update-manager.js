...
function startSecureDownload(manifest){
  const url=String(manifest?.url||'');if(!url)return typeof toast==='function'&&toast('Lien de mise à jour indisponible');
  const btn=u$('fastUpdateNow');if(btn){btn.disabled=true;btn.textContent='Ouverture du téléchargement…'}
  openExternal(url);
  setTimeout(()=>{if(btn){btn.disabled=false;btn.textContent='Télécharger la mise à jour'}},1200);
}
