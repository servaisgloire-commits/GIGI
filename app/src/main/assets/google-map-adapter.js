...
(function(){
  ...
  function color(v,f){return typeof v==='string'?v:f}
  ...
  class MapCompat{
    ...
    _init(){
      ...
      setTimeout(()=>google.maps.event.trigger(this.gmap,'resize'),350)
    }
    on(n,f){(this.events[n]||(this.events[n]=[])).push(f);if(n==='load'&&this.loaded)setTimeout(f,0);return this}
    _emit(n,a){(this.events[n]||[]).forEach(f=>{try{f(a)}catch(e){console.error(e)}})}
    resize(){if(this.gmap)google.maps.event.trigger(this.gmap,'resize')}
    isStyleLoaded(){return!!this.loaded}
    addSource(id,d){this.sources[id]=new Source(this,id,d&&d.data);return this}
    getSource(id){return this.sources[id]||null}
    removeSource(id){delete this.sources[id]}
    addLayer(l){this.layers[l.id]=l;this._drawLayer(l.id);return this}
    getLayer(id){return this.layers[id]||null}
    removeLayer(id){this._clearOverlay(id);delete this.layers[id]}
    _clearOverlay(id){const o=this.overlays[id];if(!o)return;(Array.isArray(o)?o:[o]).forEach(x=>x&&x.setMap&&x.setMap(null));delete this.overlays[id]}
    _renderLayerForSource(s){Object.keys(this.layers).forEach(id=>{if(this.layers[id].source===s)this._drawLayer(id)})}
    _drawLayer(id){
      if(!this.loaded)return;const l=this.layers[id],src=l&&this.sources[l.source];if(!l||!src)return;this._clearOverlay(id);
      const d=src.data||{},fs=d.type==='FeatureCollection'?(d.features||[]):[d],os=[];
      fs.forEach(f=>{if(!f||!f.geometry)return;if(l.type==='line'&&f.geometry.type==='LineString'){const p=l.paint||{};os.push(new google.maps.Polyline({map:this.gmap,path:(f.geometry.coordinates||[]).map(toLatLng),strokeColor:color(p['line-color'],'#0b57d0'),strokeOpacity:number(p['line-opacity'],.95),strokeWeight:number(p['line-width'],6),zIndex:20}))}else if(l.type==='circle'&&f.geometry.type==='Point'){const p=l.paint||{},c=toLatLng(f.geometry.coordinates);os.push(new google.maps.Marker({map:this.gmap,position:c,icon:{path:google.maps.SymbolPath.CIRCLE,scale:number(p['circle-radius'],8),fillColor:color(p['circle-color'],'#1677ff'),fillOpacity:1,strokeColor:color(p['circle-stroke-color'],'#fff'),strokeWeight:number(p['circle-stroke-width'],3)},zIndex:30}))}});
      this.overlays[id]=os
    }
    fitBounds(bounds,opts){if(!this.gmap)return;const b=bounds&&bounds._b?bounds._b:bounds;if(!b)return;this.gmap.fitBounds(b,opts&&opts.padding?opts.padding:40)}
    easeTo(o){if(!this.gmap)return;if(o.center)this.gmap.panTo(toLatLng(o.center));if(o.zoom!=null)this.gmap.setZoom(Number(o.zoom))}
  }
  ...
  window.FASTGoogleMaps={load:loadGoogle,retry,isReady:()=>state.ready&&!!(window.google&&google.maps),status:()=>({ready:state.ready,loading:state.loading,error:state.error,attempts:state.attempts})};
