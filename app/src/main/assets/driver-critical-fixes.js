...
async function verifyPinFast(){
  ...
  try{
    ...
    fx('fastPinVerified')?.classList.remove('hidden');
    if(input)input.disabled=true;
    if(btn){btn.disabled=true;btn.textContent='PIN vérifié ✓'}
...
function cleanupEndedRide(id,message){
  ...
  document.body.classList.remove('fast-driver-trip-active');
  fx('driverTrip')?.classList.add('hidden');
...
async function cancelRideFast(){
  const id=rideId();if(!id||cancelBusy)return;
...
function targetFromAction(button){
  const text=String(button?.textContent||'').toLowerCase();
  if(text.includes('arriv'))return 'driver_arriving';
  if(text.includes('démarrer')||text.includes('demarrer'))return 'in_progress';
  if(text.includes('dépôt')||text.includes('depot')||text.includes('termin'))return 'completed';
  return null;
}

async function fastTransition(target,button){
  const id=rideId();if(!id||transitionBusy||!target)return;
  transitionBusy=true;const old=button?.textContent||'';
  if(button){button.disabled=true;button.textContent='Confirmation…'}
  try{
    ...
    try{await fetchJson('/v1/rides/'+id+'/status',{method:'PATCH',body:JSON.stringify({status:target})},7000)}
    catch(e){if(!await statusIs(id,target))throw e}
    if(target==='driver_arriving'){
      if(button){button.textContent='PIN requis avant démarrage';button.disabled=true}
      notify('Arrivée confirmée');
...
function captureCriticalActions(e){
  ...
  if(action&&isDriver()){
    const target=targetFromAction(action);if(!target)return;
    e.preventDefault();e.stopImmediatePropagation();fastTransition(target,action);
  }
}
